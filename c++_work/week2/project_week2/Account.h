...
std::vector<double> dummybalance = {4000 , 2000 , 4000 , 5000};
...
private:
    ...
    // float balance = 100.01; // can be in decimal
public:
    ...
    bool setBalance(int amount , std::string accNum)
    {
        for (unsigned i = 0; i < dummyAccCust.size(); i++)
        {
            if (accNum == dummyAccCust[i])
            {
                dummybalance[i] -= amount;
                return true;
            }
        }
        return false;
    ...
    double getBalance(std::string accNum)
    {
        for (unsigned i = 0; i < dummyAccCust.size(); i++)
        {
            if (accNum == dummyAccountNumbers[i])
            {                
                return dummybalance[i];
            }
        }
        return -1;
    }
    ...
    std::vector<std::string> getAccountByAccountNumber(std::string accNum)
    {
        ...
        for(unsigned i =0; i<dummyAccCust.size(); i++)
        {
            ...
            {   
                ...
                temp.push_back(std::to_string(dummybalance[i]));
                break;
            }
    ...
    bool createAccount(std::string fName , std::string lName , std::string ssn)
    {
        ...
        if (Customer::createCutomer(fName , lName , ssn) == "Cannot Add Empty Fields")
        {
            return false;
        }
