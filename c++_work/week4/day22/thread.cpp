...
void simulate(int idx , Body *bod)
{
    for (int i=0; i<k; i++)
    {
        std::cout << "inside thread " << idx << std::endl;
        bod[idx].Fi(bod);
        bod[idx].calcTotalFoce(bod);
        bod[idx].updatePosition();
        bod[idx].updateVelocity();
    }
}
...
int main()
{
    ...
    for (int i = 0; i<N; i++)
    {
        thrds.push_back(std::thread(simulate , i , bod));
    }
        //joining threads
    for (int i =0; i<N; i++)
    {
